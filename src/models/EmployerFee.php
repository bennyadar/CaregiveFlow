...
class EmployerFee
{
    ...
    public function all(array $filters = [], int $limit = 25, int $offset = 0): array
    {
        ...
        return self::stripTablePrefixes($rows);
    }
...
/**
 * מנקה prefix של טבלה/alias ממפתחות תוצאות (למשל 'f.id' => 'id').
 * זה מונע Warnings ב-views כאשר PDO מוגדר להחזיר שמות טבלאות.
 */
private static function stripTablePrefixes(array $rows): array
{
    foreach ($rows as $i => $row) {
        foreach ($row as $k => $v) {
            if (strpos($k, '.') !== false) {
                $plain = substr($k, strrpos($k, '.') + 1);
                if (!array_key_exists($plain, $row)) {
                    $row[$plain] = $v;
                }
            }
        }
