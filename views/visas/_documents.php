<?php
// ===== views/visas/_documents.php =====
?>
<?php /** @var array $item, $documents, $docTypes */ ?>
<div class="card mt-4">
  <div class="card-header d-flex justify-content-between align-items-center">
    <strong>קבצים מצורפים</strong>
    <small class="text-muted">לויזה #<?= e($item['id']) ?> של העובד #<?= e($item['employee_id']) ?></small>
  </div>
  <div class="card-body">
    <form method="post" enctype="multipart/form-data" class="row gy-2 gx-2 align-items-end" action="?r=visas/upload_document">
      <?php if (function_exists('csrf_field')) csrf_field(); ?>
      <input type="hidden" name="visa_id" value="<?= e($item['id']) ?>">
      <input type="hidden" name="employee_id" value="<?= e($item['employee_id']) ?>">

      <div class="col-md-3">
        <label class="form-label">סוג קובץ</label>
        <select name="doc_type" class="form-select">
          <option value="visa" selected>ויזה</option>
          <?php foreach ($docTypes as $code => $name): if ($code==='visa') continue; ?>
        ...
        <?php else: foreach ($documents as $d): ?>
          <tr>
            ...
            <td class="text-center">
              <form method="post" action="?r=visas/delete_document" onsubmit="return confirm('למחוק את הקובץ?');">
                <?php if (function_exists('csrf_field')) csrf_field(); ?>
