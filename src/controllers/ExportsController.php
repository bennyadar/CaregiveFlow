...
//        $line .= self::strL($r['phone'] ?? '', 10);                // 305-314
    ...
    public static function piba_export(PDO $pdo)
    {
        ...
        try {
            // PIBA - Permit Extension JSON (replaces the previous XML export)
            require_once __DIR__ . '/../services/PibaPermitJsonBuilder.php';
            $builder = new PibaPermitJsonBuilder($pdo);
    ...
    /**
     * AJAX preview for PIBA JSON.
     * GET index.php?r=exports/piba_preview&employee_id=123
     * Returns JSON with: ok, filename, missing, payload, json.
     */
    public static function piba_preview(PDO $pdo): void
    {
        require_login();
        ...
        if ($employee_id <= 0) {
            http_response_code(400);
            header('Content-Type: application/json; charset=UTF-8');
            echo json_encode(['ok' => false, 'error' => 'employee_id is required'], JSON_UNESCAPED_UNICODE);
            return;
        }
        ...
        try {
            require_once __DIR__ . '/../services/PibaPermitJsonBuilder.php';
            $builder = new PibaPermitJsonBuilder($pdo);
            $res = $builder->previewForEmployee($employee_id);
