...
class EmployeesController {
    public static function index(PDO $pdo) {
        require_login();
        // === Tabs (לפי המוקאף) ===
        // list = רשימה, stats = סטטיסטיקה
        $tab = $_GET['tab'] ?? 'list';
        if (!in_array($tab, ['list', 'stats'], true)) {
            $tab = 'list';
        }
        ...
        $m = new Employee($pdo);
        $rows = $m->search($filters, $limit, $offset);
        $total = $m->searchCount($filters);
        $pages = (int)ceil(($total ?: 0) / $limit);

        // ===== KPI (סטטוסים) לתצוגה דשבורדית בעמוד הרשימה =====
        // הערה: מחושב עבור כלל העובדים (לא רק לעמוד הנוכחי), כדי שהמספרים יהיו שימושיים.
        $expiryKpis = self::employee_expiry_kpis($pdo, 30);

        // ===== Options לפילטרים (Dropdowns) =====
        // טעינת רשימת מעסיקים עבור פילטר "מעסיק בשיבוץ פעיל"
        $employersOptions = [];
        try {
            $stmt = $pdo->query("SELECT id, COALESCE(NULLIF(company_name,''), TRIM(CONCAT(last_name,' ',first_name))) AS name FROM employers ORDER BY name");
            $employersOptions = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
        } catch (Throwable $e) {
            $employersOptions = [];
        }
    ...
    public static function show(PDO $pdo) {
        ...
        // ===== Summary (תוקפים) לתצוגה דשבורדית בראש כרטיס העובד =====
        $summary_expiry = self::expiries_for_employees($pdo, [$id])[(int)$id] ?? [
    ...
    /**
     * KPI לעובדים: מחזיר ספירות של תוקפים (פג / 30 יום הקרובים).
     *
     * הערה: השאילתות נבנו כך שיחשבו "תוקף רלוונטי" פר עובד:
     * - דרכון: MAX(expiry_date) מטבלת employee_passports
     * - ויזה/ביטוח: הקרוב הבא (MIN>=היום), ואם אין – המאוחר ההיסטורי (MAX)
     */
    private static function employee_expiry_kpis(PDO $pdo, int $soonDays = 30): array
    {
        $soonDate = (new DateTime('today'))->modify('+' . $soonDays . ' days')->format('Y-m-d');

        // דרכונים: MAX(expiry_date) per employee
        $sqlPassport = "
            SELECT
              SUM(t.expiry_date < CURDATE()) AS expired,
              SUM(t.expiry_date >= CURDATE() AND t.expiry_date <= :soon) AS soon
            FROM (
        ...
        ";
        $stP = $pdo->prepare($sqlPassport);
        $stP->execute([':soon' => $soonDate]);
        $p = $stP->fetch(PDO::FETCH_ASSOC) ?: ['expired' => 0, 'soon' => 0];

        // ויזות: next future else last
        $sqlVisa = "
            SELECT
              SUM(t.expiry_date < CURDATE()) AS expired,
              SUM(t.expiry_date >= CURDATE() AND t.expiry_date <= :soon) AS soon
            FROM (
              SELECT employee_id,
                     COALESCE(MIN(CASE WHEN expiry_date >= CURDATE() THEN expiry_date END), MAX(expiry_date)) AS expiry_date
              FROM visas
              GROUP BY employee_id
            ) t
        ";
        $stV = $pdo->prepare($sqlVisa);
        $stV->execute([':soon' => $soonDate]);
        $v = $stV->fetch(PDO::FETCH_ASSOC) ?: ['expired' => 0, 'soon' => 0];

        // ביטוחים: next future else last
        $sqlIns = "
            SELECT
              SUM(t.expiry_date < CURDATE()) AS expired,
              SUM(t.expiry_date >= CURDATE() AND t.expiry_date <= :soon) AS soon
        ...
        $stI->execute([':soon' => $soonDate]);
        $i = $stI->fetch(PDO::FETCH_ASSOC) ?: ['expired' => 0, 'soon' => 0];

        return [
            'soonDays' => $soonDays,
            'passport' => ['expired' => (int)$p['expired'], 'soon' => (int)$p['soon']],
            'visa'     => ['expired' => (int)$v['expired'], 'soon' => (int)$v['soon']],
            'insurance'=> ['expired' => (int)$i['expired'], 'soon' => (int)$i['soon']],
        ];
    }
