<?php /** @var array $items, $employees, $status_codes, $type_codes; @var int $page,$pages,$total; */ ?>
<?php require __DIR__ . '/../layout/header.php'; ?>
<div class="container" dir="rtl">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="m-0">ביטוחים</h3>
        <a class="btn btn-primary" href="?r=insurances/create">+ ביטוח חדש</a>
    </div>

    <!-- סינון -->
    <form class="row g-2 mb-3" method="get" action="">
        <input type="hidden" name="r" value="insurances">
        ...
        <div class="col-md-3">
            ...
            <select name="status" class="form-select">
                ...
                <?php endforeach; ?>
                <?php // אפשרות לפילטר "פג" כנגזר מהתאריך ?>
        ...
        <div class="col-md-2">
            <label class="form-label">חיפוש (מס׳ פוליסה/מבטח)</label>
    ...
    <div class="table-responsive">
        <table class="table table-striped align-middle">
            ...
            <?php foreach ($items as $row): ?>
                <?php 
                    $derived = InsuranceService::derivedStatusCode($row['status_code'] ?? null, $row['expiry_date'] ?? null);
                    $days    = InsuranceService::daysUntilExpiry($row['expiry_date'] ?? null);
                ?>
                <tr>
                    <td><?= e($row['id']) ?></td>
                    <td>
                    ...
                    <td><?= e($row['policy_number']) ?></td>
                    <td><?= e($row['insurer_name']) ?></td>
                    <td><?= e($row['type_name'] ?? ($row['insurance_type_code']!==null?($type_codes[(int)$row['insurance_type_code']]??''):'')) ?></td>
                    ...
                    <td class="text-end">
                        <a class="btn btn-sm btn-outline-secondary" href="?r=insurances/view&id=<?= e($row['id']) ?>">צפייה</a>
                        <a class="btn btn-sm btn-outline-primary" href="?r=insurances/edit&id=<?= e($row['id']) ?>">עריכה</a>
