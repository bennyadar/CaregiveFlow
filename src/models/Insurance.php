...
class Insurance
{
    ...
    public function create(array $data): int
    {
        ...
        if (!empty($data['is_primary'])) {
            $stm2 = $this->pdo->prepare("UPDATE employee_insurances SET is_primary = 0 WHERE employee_id = :emp AND id <> :id");
    ...
    public function all(array $filters = [], int $limit = 25, int $offset = 0): array
    {
        ...
        // מבנה זהה למודול דרכונים: i.* + שדות עובדים + שמות קודים.
        $sql = "SELECT i.*, e.first_name, e.last_name, e.passport_number AS employee_passport,
             ...
             LEFT JOIN insurance_type_codes   tc ON tc.insurance_type_code   = i.insurance_type_code
                  $whereSql
              ORDER BY i.is_primary DESC, COALESCE(i.expiry_date, '9999-12-31') ASC, i.id DESC
