<?php /** @var array $rows, $employees; @var int $page,$pages,$total; */ ?>
    ...
    <div class="table-responsive">
        <table class="table table-striped align-middle">
            ...
            <?php foreach ($rows as $row): ?>
                <?php 
                    // הגנה מפני שורות שמגיעות בלי כל העמודות (למשל אם שאילתת הרשימה אינה כוללת status/expiry_date)
                    $status = $row['status'] ?? null;
                    $expiry = $row['expiry_date'] ?? null;

                    $derived = VisaService::derivedStatus($status, $expiry);
                    $days = VisaService::daysUntilExpiry($expiry);
                ?>
                <tr>
	                    <td><?= e($row['id'] ?? '') ?></td>
                    <td>
